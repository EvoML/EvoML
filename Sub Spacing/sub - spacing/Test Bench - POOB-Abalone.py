...
def test_bench_main(dataSetChoice):
    ...
    for i in range(dataSetChoice,dataSetChoice+1):
        ...
        #                                   , base_estimator = regressor, test_frac=0.7)
...
def get_hof_fitness_unseen(clf, unseen_x, unseen_y):
    ...
    return fitness_f, unseen_f, first_mse, hof_mse, first_fitness, hof_fitness, feat_cnt, feat_avg_cnt
...
g_data = []
g_A = []
for dataSetChoice in range(1,2):
    f_f = []
    u_f = []
    csv_f = []
    name = 'POOB-c-datSet-'+str(dataSetChoice)
    for i in range(0,50):
        print i
        A = test_bench_main(dataSetChoice)
        fitness_f, unseen_f, first_mse, hof_mse, first_fitness, hof_fitness, feat_cnt, feat_avg_cnt = get_hof_fitness_unseen(A, A.unseen_x, A.unseen_y)
