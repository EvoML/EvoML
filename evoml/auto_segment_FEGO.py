...
"""
Algorithm - Fit_Ensemble_Global_oob (FEGO)
Fitness is measured that of entire ensemble, and on the global oob.
Global oob is calcuated by using the union of the samples used to train each model
in the ensemble and then finding the ~ of it from the entire training data.

mutators: segment_mutator_EG
...
from evaluators import evalOneMax_KNN_EG, eval_ensemble_oob_KNN_EG
...
class BasicSegmenterEG_FEGO(BaseEstimator, RegressorMixin):
    """
    ...
    def __init__(self, n = 10, test_size = 0.2, 
                ...
                init_sample_percentage = 1, indpb =0.20, crossover_func = tools.cxTwoPoint, statistics = True,
...
#                                                           test_size=self.test_size)
        # There is no test created in this. Global oob is used.
  ...
  #      df_test = pd.concat([X_test, y_test], axis = 1)
        ...
        toolbox.register("evaluate", eval_ensemble_oob_KNN_EG, df = df, base_estimator = self.base_estimator, n_votes = self.n_votes)
        toolbox.register("mate", self.crossover_func)
        #Can pool from the entire datapool.
        ...
        algorithms.eaSimple(pop, toolbox, cxpb=self.cxpb, mutpb=self.mutpb, ngen=self.ngen, stats=stats, halloffame= hof, verbose = True)
