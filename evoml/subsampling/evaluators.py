...
def evalOneMax_KNN_EG(individual, df_test, base_estimator, n_votes):
    """
    This is used in FEGT - Fitness Ensemble Global Test
    ...
    n_votes: int, required
        The number of nearest to row in X dataframes to be used for voting while predicting
        each row in X. Experimental. If n_votes equals number of models in ensembles, average
        of all model's prediction is taken.
    ...
    for row in df_test.iloc[:, 0:-1].values:
        ...
        # Get the closest n_votes dataframes to 'row'
...
def eval_each_model_oob_KNN_EG(individual, df, base_estimator, n_votes):
    ...
    private oob - or out of bag sample.

    Used in FEMPO.

    Replicates how a random forest works.

    Does not consider voting in the fitness.
...
def eval_each_model_PT_KNN_EG(individual, df, base_estimator, n_votes):
    ...
    for eg_ in individual:
        ...
        # rmse is precalculated in EG if private_test = True for efficiency.
