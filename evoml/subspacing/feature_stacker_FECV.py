...
Evaluator: evalOneMax3 for regression
           evalOneMax_class2 for classification 
Mutator: mutate_feat
"""
...
class FeatureStackerFECV(BaseEstimator,RegressorMixin):
    ...
    def __init__(self, test_size=0.30,N_population=40,N_individual=5,featMin=1,featMax=None,
     ...
     crossover_func = tools.cxTwoPoint, model_type = 'regression', maxOrMin = 1, verbose_flag = True, folds_CV = 3, indiv_replace_flag = False):
    ...
    def get_indiv_sample_bag(self,data,output,base_estimator, indiv_replace_flag):        
        '''
        This function generates the chromosomes for the population
        '''
    ...
    def fit(self, X, y):
        ...
        toolbox.register("attr_bool", self.get_indiv_sample_bag, data=X, output=y, base_estimator=self.base_estimator, indiv_replace_flag=self.indiv_replace_flag)
        toolbox.register("individual", tools.initRepeat, creator.Individual, toolbox.attr_bool, n=self.N_individual)
        toolbox.register("population", tools.initRepeat, list, toolbox.individual)
        if(self.model_type=='regression'):
            toolbox.register("evaluate", evalOneMax3, folds_CV = self.folds_CV, y_f = y)
        elif(self.model_type=='classification'):
            toolbox.register("evaluate", evalOneMax_class2, folds_CV = self.folds_CV, y_f = y)
    ...
    def predict(self, input_features):
        ...
        if(self.model_type == 'regression'):
            ...
            for i in range(0,len(indiv)):
                chromosome = indiv[i]
                mod_ = chromosome.estimator
                predict_1ist.append(mod_.predict(input_features[list(chromosome.X.columns.values)]))
            result = [];
            for i in range(0,len(indiv)):
                if(i==0):
                    result = predict_1ist[0]
                else:
                    result = result + predict_1ist[i]
            ...
            for i in range(0,len(indiv)):
                chromosome = indiv[i]
                mod_ = chromosome.estimator
                predict_list.append(mod_.predict(input_features[list(chromosome.X.columns.values)]))
