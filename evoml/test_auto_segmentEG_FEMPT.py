...
def main():
    ...
    clf = BasicSegmenterEG_FEMPT(ngen=30, init_sample_percentage = 0.5, n_votes=10, n = 10, base_estimator = base_estimator, unseen_x = X_test, unseen_y = y_test)
...
def get_hof_fitness_unseen(clf, unseen_x, unseen_y, run = None):
    ...
    if run != None:
        plt.savefig('./Pictures/FEMPT Without Replacement/Changing fitness with gens - run {0}'.format(run))
        plt.close()
    else:
        plt.show()
...
def exp():
    ...
    pd.DataFrame(
        ...
        'gen_first': gen_firsts
                ...
                }).to_csv('FEMPT_experiment_runs_without_replacement.csv')
    # plt.hist(corr_coefs)