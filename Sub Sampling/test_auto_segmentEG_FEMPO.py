...
def get_hof_fitness_unseen(clf, unseen_x, unseen_y, run = None, dataset_name = ''):
    ...
    # Replace this line for the full experiment. Also, modify EASimple.
    # pops = pops[1:]
    # print pops
    # pops = pops[0:]
    ...
    # Uncomment these lines for full experiment
    ...
    # if run != None:
    #     # plt.savefig('./Pictures/FEMPO_{0}/Changing fitness with gens - run {1}'.format(dataset_name, run))
    #     # plt.close()
...
def exp(dataset_name, runs):
    ...
    pd.DataFrame(
        ...
        'number_of_uniques_vars' : number_of_uniques_vars
                ...
                }).to_csv('FEMPO_experiment_runs_{0}.csv'.format(dataset_name))
