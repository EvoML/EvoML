...
def main(X, y, test_size = 0.1):
    ...
    if run_infy == True:
        ...
        clf = BasicSegmenterEG(ngen=30, n_population = 1, init_sample_percentage = 0.5, n_votes=10, n = 10, base_estimator = base_estimator)
        clf.fit(X_train, y_train)
        print clf.score(X_test,y_test)


    clf = BasicSegmenterEG(ngen=30, init_sample_percentage = 1, n_votes=10, n = 10, base_estimator = base_estimator)
    clf.fit(X_train, y_train)
    print clf.score(X_test,y_test)
...
#     if run != None:
#         plt.savefig('./Pictures/FEPT_2/Changing fitness with gens - run {0}'.format(run))
#         plt.close()
#     else:
#         plt.show()
...
#                 }).to_csv('FEPT_experiment_runs_2.csv')
...
def get_hof_fitness_unseen(clf, unseen_x, unseen_y, run = None, dataset_name = ''):
    ...
    # if run != None:
    #     plt.savefig('./Pictures/FEPT_{0}/Changing fitness with gens - run {1}'.format(dataset_name, run))
    #     plt.close()
    # else:
    #     plt.show()
...
def exp(dataset_name, runs):
    ...
    pd.DataFrame(
        ...
        'number_of_uniques_vars' : number_of_uniques_vars
                ...
                }).to_csv('FEPT_experiment_runs_{0}_10.csv'.format(dataset_name))
